import { deleteUserAndSessions, getTicket } from "../qlik-utils.mjs";

export async function redirectToQlik(userdir, user, attributes, { targetId, proxyRestUri }, res) {
    if (!targetId || !proxyRestUri) return res.sendStatus(400);

    const ticketData = await getTicket(proxyRestUri, userdir, user, attributes, targetId);

    if (!ticketData || !ticketData.Ticket || !ticketData.TargetUri) return res.sendStatus(401);

    const { Ticket, TargetUri } = ticketData;

    // Redirection
    const redirectURI =
        TargetUri.indexOf("?") > 0 ? `${TargetUri}&qlikTicket=${Ticket}` : `${TargetUri}?qlikTicket=${Ticket}`;
...
export function userFromRequest(req) {
    ...
    return {
        ...
        Attributes: [{ photo: photos && photos.length > 0 ? photos[0].value : null }, { userName: displayName }],
    };
}
...
export async function webLoginSuccessHandler(req, res) {
    ...
    if (!ticketData || !ticketData.Ticket) return res.sendStatus(401);
    ...
    const redirectURI =
        redirect.indexOf("?") > 0 ? `${redirect}&qlikTicket=${Ticket}` : `${redirect}?qlikTicket=${Ticket}`;
